...
if(isset($_POST["import"]))
{
 ...
 if(in_array($extension, $allowed_extension)) //check selected file extension is present in allowed extension array
 {
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   $stud_school_id_number = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
   $stud_address = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
   $teacher_department =$worksheet->getCellByColumnAndRow(5, $row)->getValue();
   $teacher_email = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
   $teacher_contact = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
   $teacher_img = 'user.png';
   $permitted_chars = '0123456789abcdefghijklmnopqrstuvwxyz';
   $teacher_password =  substr(str_shuffle($permitted_chars), 0, 8);
   $teacher_password =  md5($teacher_password );
...
<div class="modal fade" id="excel_send" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document"1>
      <div class="landing-modal modal-content">
        ...
        <p  class="text-center px-5" style="color: rgb(112, 112, 112)" >Once file Imported, the List will automatically saved to teachers List.</p>
            ...
            <div class="col-sm-12 col-lg-6 right-side-form pr-lg-5">
                <div class="d-flex justify-content-center justify-content-lg-end">
                  ...
                  <img id="user_image" src="" alt="" style="height: 80px; border: solid 1px grey; border-radius: 50%; width: 80px !important;">
                    <input name="img" type="file" id="file-field" accept="image/*"  onchange="previewImage(event)" style="display: none;">
